...
class TarjanAlgorithmNoMaps extends FlatSpec with Matchers {
  ...
  "Tarjan " should "report an algebraic loop" in {
    val g = new TarjanGraphNoMaps[Z](allEdges.elements)
    assert(g.hasCycle)
  }
  ...
  "Tarjan Graph with two loops" should "report two algebraic loops" in {
    ...
    val g = new TarjanGraphNoMaps[Z](edges.elements)
    assert(g.hasCycle)
  ...
  "Tarjan BigGrahp 100+ edges" should "report AN algebraic loop" in {
    ...
    val g = new TarjanGraphNoMaps[Z](alotOfEdges.elements)
    assert(g.hasCycle)
  }
  ...
  "Topological sort on a more advanced graph" should "report the topological order of the graph" in {
    ...
    val g = new TarjanGraphNoMaps[C](graphEdges.elements)
    assert(!g.hasCycle)
